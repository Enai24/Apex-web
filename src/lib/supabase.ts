...
// Type mapping for table names to their types
type TableTypeMap = {
  clients: Client;
  jobs: Job;
  time_entries: TimeEntry;
...
// Mock database operations with proper generics
export const mockDB = {
  from: <T extends keyof Database>(table: T) => ({
    select: () => ({
      order: (column: string, { ascending = true } = {}) => {
        const data = [...db[table]] as TableTypeMap[T][];
    ...
    insert: (data: Partial<TableTypeMap[T]>) => {
      const newItem = { id: String(db[table].length + 1), ...data } as TableTypeMap[T];
      (db[table] as TableTypeMap[T][]).push(newItem);
      return Promise.resolve({ data: newItem, error: null });
    },
    update: (data: Partial<TableTypeMap[T]> & { id: string }) => {
      const index = db[table].findIndex((item: any) => item.id === data.id);
      if (index !== -1) {
        (db[table] as any)[index] = { ...db[table][index], ...data };
        return Promise.resolve({ data: db[table][index] as TableTypeMap[T], error: null });
      }
