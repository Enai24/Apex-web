...
export default function JobSeekersPage() {
  ...
  return (
    <div className="bg-background">
      {/* Hero section */}
      <div className="relative bg-navy py-24 sm:py-32">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <div className="mt-10 flex items-center justify-center gap-x-6">
              <Link
                href="#available-jobs"
                className="rounded-sm bg-white px-5 py-3 text-base font-semibold text-navy shadow-sm hover:bg-oat-medium transition-colors duration-300"
              >
                Browse Jobs
              </Link>
              <Link
      ...
      <div id="available-jobs" className="py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl lg:text-center">
            <h2 className="eyebrow">Available Opportunities</h2>
            <p className="mt-2 text-3xl font-medium tracking-tight text-navy sm:text-4xl">
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-2">
            {jobCategories.map((category) => (
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-sm font-medium ${selectedCategory === category.id
                    ? 'bg-orange-red text-white'
                    : 'bg-muted text-navy hover:bg-oat-medium'
                  }`}
              >
                {category.name}
              </button>
          ...
          <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-2">
            {filteredJobs.map((job) => (
              <div key={job.id} className="card p-6 hover:shadow-card transition-all">
                <div className="flex justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">{job.title}</h3>
                    <p className="mt-1 text-sm text-gray-500">{job.company} • {job.location}</p>
                  </div>
                  <button className="flex h-10 w-10 items-center justify-center rounded-full bg-muted text-teal-gray hover:bg-oat-medium hover:text-orange-red">
                    <Bookmark className="h-5 w-5" />
                  </button>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  <span className="inline-flex items-center rounded-sm bg-oat-medium px-2.5 py-1 text-xs font-medium text-navy">
                    {job.type}
                  </span>
                ...
                <div className="mt-6 flex justify-between items-center">
                  <Link
                    href={`/jobs/${job.id}`}
                    className="text-sm font-medium text-orange-red hover:opacity-80 flex items-center group"
                  >
                    View details <ArrowRight className="ml-1 h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" />
                  </Link>
                  <button className="rounded-sm bg-orange-red px-3 py-1.5 text-sm font-medium text-white hover:opacity-90">
                    Apply
                  </button>
                </div>
              </div>
      ...
      <div id="career-advice" className="bg-oat-medium py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl lg:text-center">
            <h2 className="eyebrow">Career Resources</h2>
            <p className="mt-2 text-3xl font-medium tracking-tight text-navy sm:text-4xl">
            ...
            {careerArticles.map((article) => (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-y-1 overflow-hidden text-sm leading-6 text-gray-300">
                  <span className="inline-flex items-center rounded-full bg-orange-red/10 px-3 py-1 text-xs font-medium text-orange-red ring-1 ring-inset ring-orange-red/20">
                    {article.category}
                  </span>
                </div>
      ...
      <div className="bg-white py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-12 gap-y-16 lg:mx-0 lg:max-w-none lg:grid-cols-2 lg:items-start">
            <div>
              <h2 className="eyebrow">Employee Benefits</h2>
              <p className="mt-2 text-3xl font-medium tracking-tight text-navy sm:text-4xl">
                ...
                {employeeBenefits.map((benefit) => (
                  <div key={benefit.title} className="relative pl-10">
                    <dt className="text-base font-semibold leading-7 text-gray-900">
                      <div className="absolute left-0 top-0 flex h-8 w-8 items-center justify-center rounded-sm bg-orange-red">
            ...
            <div className="relative">
              ...
              <div className="absolute -bottom-6 -left-6 rounded-sm bg-orange-red p-8 shadow-lg">
