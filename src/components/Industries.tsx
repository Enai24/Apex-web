...
export default function Industries() {
  ...
  const localizedIndustries = useMemo(
    () => translateData(industrySolutions, { skipKeys: ['id', 'image', 'iconName', 'caseStudyId'] }),
    [translateData]
  );
      ...
      <section className="bg-oat-medium py-20 border-b border-border">
        <div className="container">
          <div className="max-w-3xl">
            <span className="eyebrow mb-4 block">{translate('SECTOR EXPERTISE')}</span>
            <h1 className="text-navy text-[40px] md:text-[64px] font-medium leading-tight mb-6">
              {translate('Sector-Specific Workforce')}<br />{translate('Solutions in India')}
            </h1>
            <p className="text-teal-gray text-lg md:text-xl leading-relaxed">
              {translate("India's most trusted recruitment partner for Manufacturing, IT, Healthcare, and BFSI sectors.")}
              {' '}
              {translate('We solve operational complexities with AI-powered talent intelligence.')}
            </p>
          </div>
        </div>
      </section>
              ...
              return (
                <div key={industry.id} className="flex flex-col md:flex-row gap-8 p-10 bg-oat border border-border group hover:bg-white hover:shadow-premium transition-all duration-500">
                  ...
                  <div className="md:w-2/3 flex flex-col justify-center">
                    ...
                    >
                      {translate('Explore Solution')} <ArrowRight size={14} className="ml-2" />
                    </Link>
                  </div>
                </div>
              );
        ...
        <div className="container relative z-10">
          <div className="grid md:grid-cols-3 gap-12">
            <div>
              ...
              <h4 className="text-xl font-medium mb-4">{translate('100% Compliance Guarantee')}</h4>
              <p className="text-white/60 text-sm">{translate('We take full statutory responsibility, ensuring your business is audit-ready 24/7 across every Indian state.')}</p>
            </div>
            <div>
              <Users className="text-orange-red mb-6" size={40} />
              <h4 className="text-xl font-medium mb-4">{translate('Pan-India Network')}</h4>
              <p className="text-white/60 text-sm">{translate('Access talent across 25+ cities and Tier-2 hubs through our localized recruitment ecosystem.')}</p>
            </div>
            <div>
              <CheckCircle2 className="text-orange-red mb-6" size={40} />
              <h4 className="text-xl font-medium mb-4">{translate('Domain Experts')}</h4>
              <p className="text-white/60 text-sm">{translate('Our teams understand the chemistry of pharma, the cadence of manufacturing, and the code of technology.')}</p>
            </div>
          </div>
        </div>
      </section>
