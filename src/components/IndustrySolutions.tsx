...
export default function IndustrySolutions({ industry }: Props) {
    const { translate, translateData, t } = useI18n();
    const localizedIndustry = useMemo(
        () => translateData(industry, { skipKeys: ['id', 'image', 'iconName', 'caseStudyId'] }),
        [industry, translateData]
    );

    return (
        <div className="flex min-h-screen flex-col bg-background font-sans">
            <SEO
                title={translate('{industry} Industry Solutions | Apex Enterprises', { industry: localizedIndustry.name })}
                description={localizedIndustry.hero.description}
            />
            ...
            <section className="relative bg-navy py-16 lg:py-24 border-b border-border overflow-hidden">
                <div className="absolute inset-0 opacity-10 pointer-events-none">
                    <Image
                        src={localizedIndustry.hero.image}
                        alt=""
                        fill
                ...
                <div className="container mx-auto px-6 lg:px-8 relative z-10">
                    <div className="max-w-4xl">
                        <span className="font-mono text-sm text-orange-red uppercase tracking-widest mb-4 block">
                            {localizedIndustry.hero.subtitle}
                        </span>
                        <h1 className="text-white text-[40px] md:text-[56px] lg:text-[64px] font-medium leading-[1.1] mb-6">
                            {localizedIndustry.hero.title}
                        </h1>
                        <p className="text-white/80 text-lg md:text-xl leading-relaxed max-w-2xl mb-10">
                            {localizedIndustry.hero.description}
                        </p>
                        <div className="flex flex-wrap gap-4">
                            <Link href="/contact" className="btn-primary">
                                {translate('Talk to an Expert')}
                            </Link>
                            {localizedIndustry.caseStudyId && (
                                <Link href={`/portfolio#${localizedIndustry.caseStudyId}`} className="btn-secondary border border-white/20">
                                    {translate('View Story')}
                                </Link>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid lg:grid-cols-2 gap-16 items-center">
                        <div>
                            <h2 className="text-navy text-3xl md:text-4xl font-medium mb-6">
                                {localizedIndustry.overview.title}
                            </h2>
                            <p className="text-teal-gray text-lg leading-relaxed">
                                {localizedIndustry.overview.content}
                            </p>
                        </div>
                        <div className="bg-oat-medium0/20 p-8 lg:p-12 border-l-4 border-orange-red">
                            <h4 className="text-navy font-bold uppercase tracking-widest text-xs mb-8">{translate('Strategic Benefits')}</h4>
                            <ul className="space-y-6">
                                {localizedIndustry.benefits.items.map((benefit, idx) => (
                ...
                <div className="container mx-auto px-6 lg:px-8">
                    <div className="text-center mb-16">
                        <h2 className="text-navy text-3xl md:text-4xl font-medium">
                            {localizedIndustry.challenges.title}
                        </h2>
                    </div>
                    <div className="grid md:grid-cols-3 gap-8">
                        {localizedIndustry.challenges.items.map((challenge, idx) => (
                ...
                <div className="container mx-auto px-6 lg:px-8">
                    <div className="max-w-3xl mb-16">
                        <h2 className="text-navy text-[32px] md:text-[48px] font-medium leading-tight">
                            {localizedIndustry.solutions.title}
                        </h2>
                    </div>

                    <div className="grid md:grid-cols-3 gap-12">
                        {localizedIndustry.solutions.items.map((solution, idx) => {
                            ...
                            return (
                                <div key={idx} className="group flex flex-col">
                                    ...
                                    <Link href="/contact" className="mt-auto flex items-center text-xs font-bold uppercase tracking-widest text-navy hover:text-orange-red transition-colors">
                                        {translate('Learn how it works')} <ArrowRight size={14} className="ml-2" />
                                    </Link>
                                </div>
                            );
                        })}
            ...
            <section className="bg-navy py-20 relative overflow-hidden">
                ...
                <div className="container mx-auto px-6 lg:px-8 text-center relative z-10">
                    <h2 className="text-white text-3xl font-medium mb-12">
                        {translate('Ready to transform your {industry} operations?', { industry: localizedIndustry.name.toLowerCase() })}
                    </h2>
                    <div className="flex flex-col sm:flex-row gap-6 justify-center">
                        <Link href="/contact" className="bg-orange-red text-white px-10 py-5 font-bold uppercase tracking-widest text-sm hover:scale-105 transition-transform shadow-lg">
                            {translate('Consult with an Expert')}
                        </Link>
                        <Link href="/industries" className="border border-white/20 text-white px-10 py-5 font-bold uppercase tracking-widest text-sm hover:bg-white/10 transition-colors">
                            {translate('Explore other Industries')}
                        </Link>
                    </div>
                </div>
            </section>
            ...
            <div className="bg-white py-4 border-t border-border">
                <div className="container mx-auto px-6 lg:px-8">
                    <nav className="flex text-xs font-mono uppercase tracking-widest text-teal-gray">
                        <Link href="/" className="hover:text-navy">{t('footer.home')}</Link>
                        <span className="mx-2">/</span>
                        <Link href="/industries" className="hover:text-navy">{t('footer.industries')}</Link>
                        <span className="mx-2">/</span>
                        <span className="text-navy font-bold">{localizedIndustry.name}</span>
                    </nav>
                </div>
            </div>
